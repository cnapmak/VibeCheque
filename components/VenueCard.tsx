...
export function VenueCard({ venue }: VenueCardProps) {
  ...
  return (
    <Link href={`/venue/${venue.id}`}>
      <div
        className={`group rounded-2xl border-2 overflow-hidden hover:shadow-lg transition-all duration-200 hover:-translate-y-0.5 cursor-pointer ${
        ...
        {venue.imageUrl ? (
          ...
          <div
            className={`h-28 w-full flex items-center justify-center text-5xl ${
              vibe ? vibe.iconBg : "bg-gray-200/60"
            }`}
          >
            {vibe ? vibe.emoji : "📍"}
          </div>
